...
export const AuthModal: React.FC = () => {
  ...
  const demoAccounts = isAdminMode
    ? [
        { label: 'admin@demo.com / admin123', email: 'admin@demo.com', pass: 'admin123' },
        { label: 'officer@mpcb.gov.in / admin123', email: 'officer@mpcb.gov.in', pass: 'admin123' },
      ]
    : [
        { label: 'user@demo.com / user123', email: 'user@demo.com', pass: 'user123' },
        { label: 'rajesh@apexind.in / user123', email: 'rajesh@apexind.in', pass: 'user123' },
      ];
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-md p-4 overflow-y-auto">
        ...
        <div className="bg-white rounded-b-3xl shadow-2xl p-6 relative">
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5">
            ...
            {!isAdminMode && (
              <button
                ...
                className="w-full py-3 border-2 border-slate-200 hover:border-slate-300 hover:bg-slate-50 text-slate-700 text-sm font-bold rounded-xl transition flex items-center justify-center space-x-2 cursor-pointer"
              >
          ...
          <div className="mt-5 pt-4 border-t border-slate-100">
            <p className="text-[10px] text-slate-400 text-center mb-2 uppercase tracking-wider font-bold">
              {isAdminMode ? 'Demo Admin Credentials' : 'Demo Test Accounts'}
            </p>
              ...
              {demoAccounts.map((acc) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (!isAdminMode) setFormMode('signin');
                  }}
                  className="px-3 py-1.5 text-[10px] font-bold rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 transition cursor-pointer"
                >
                  {acc.label} ({acc.email})
                </button>
              ))}
          ...
          <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-500">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-emerald-700 font-mono text-[10px]">Strict 1 Email = 1 Account</span>
            </div>
        ...
        <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm bg-white rounded-2xl shadow-2xl p-6 border border-slate-200 space-y-4 animate-in zoom-in-95">
            ...
            <div className="space-y-2">
              <button
                onClick={() => handleSelectGoogleAccount('user@demo.com', 'Demo Business User')}
                className="w-full p-2.5 rounded-xl border border-slate-200 hover:bg-slate-50 flex items-center gap-3 text-left transition cursor-pointer"
              >
                <div className="w-8 h-8 rounded-full bg-blue-600 text-white font-bold flex items-center justify-center text-xs">
                  DU
                </div>
                <div>
                  <div className="text-xs font-bold text-slate-900">Demo User</div>
                  <div className="text-[11px] text-slate-500">user@demo.com</div>
                </div>
              </button>
            </div>
